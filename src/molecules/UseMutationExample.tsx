...
import { CAST_VOTE } from '../grapgql/mutations'
...
const UseMutationExample: React.FC = () => {
  ...
  const [castVote, { loading: mutationLoading, error: mutationError, data }] = useMutation(CAST_VOTE, {onError: (error) => console.log(error.stack)});  
  ...
  console.log(data)
  return (
    <Button onClick={() => castVote({ variables: {votationId, alternativeId} })} colorScheme='blue'>Avgi stemme</Button>
  )
}
